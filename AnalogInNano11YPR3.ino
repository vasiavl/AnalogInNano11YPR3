...
// по ходу где сохраняешь то преед редактором и ставишь
//  сохраним в мастера
//тупо в ветку А добавляем дополнительные строчки
//остальные с троки скопированы из мастера посмотрим сольются ли
// тупняк продолжается правим в ГИТе смотрим как зальется на локалку
//строки только добавляем
#define Button_1      2//8
...
const int analogInPitch
Pin = A4; // пин для резистора по Pitch
